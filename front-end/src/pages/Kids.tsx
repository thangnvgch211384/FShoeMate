...
import { Filter, SlidersHorizontal, Grid3X3, List, Baby, X, Check } from "lucide-react"
...
export default function Kids() {
  ...
  useEffect(() => {
    async function loadAllProducts() {
      try {
        const response = await fetchProducts({ targetAudience: "kids" })
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const params: any = { targetAudience: "kids" }
  ...
  const productsWithoutCategoryFilter = products.filter((product) => {
    ...
    if (ageGroup !== "all" && product.sizes) {
      ...
      switch (ageGroup) {
        case "toddler":
          shouldInclude = sizeNumbers.some(size => size >= 20 && size <= 26)
          break
        case "littleKids":
          shouldInclude = sizeNumbers.some(size => size >= 27 && size <= 34)
          break
        case "teens":
          shouldInclude = sizeNumbers.some(size => size >= 35 && size <= 40)
          break
        default:
          shouldInclude = true
      }
  ...
  const allAvailableSizes = [...new Set(products.flatMap(p => p.sizes || []))].sort((a, b) => {
  ...
  const availableSizes = useMemo(() => {
    if (ageGroup === "all") return allAvailableSizes
    ...
    const range = sizeRanges[ageGroup]
    if (!range) return allAvailableSizes
    
    return allAvailableSizes.filter(size => {
      const numSize = parseFloat(size)
      if (isNaN(numSize)) return false
      return numSize >= range[0] && numSize <= range[1]
    })
  }, [allAvailableSizes, ageGroup])
  ...
  // Calculate category counts by category name (normalized) for Kids' page
  ...
  const hasActiveFilters = selectedSize !== null || selectedColors.length > 0 || selectedCategories.length > 0 || selectedBrands.length > 0 || priceRange[1] < 2000000 || ageGroup !== "all"

  const ageGroups = [
    { id: "all", label: "All", range: "All age groups" },
    { id: "toddler", label: "Toddler (1-3 years)", range: "Size 20-26" },
    { id: "littleKids", label: "Little Kids (4-10 years)", range: "Size 27-34" },
    { id: "teens", label: "Teens (11-16 years)", range: "Size 35-40" }
  ]
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero Section */}
      <section className="py-16 bg-gradient-to-br from-warning/10 via-background to-success/10">
        <Container>
          <div className="text-center max-w-3xl mx-auto">
            <div className="flex items-center justify-center gap-3 mb-4">
              <div className="w-16 h-16 bg-gradient-warning rounded-2xl flex items-center justify-center">
                <Baby className="h-8 w-8 text-white" />
              </div>
              <h1 className="text-5xl font-bold">
                Shoes for <span className="gradient-text">Kids</span>
              </h1>
            </div>
            <p className="text-xl text-muted-foreground mb-8">
              Safe and comfortable kids' shoe collection with fun colors and cute designs
            </p>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    {ageGroups.map((group) => (
                      <div
                        ...
                        className={cn(
                          ...
                          ageGroup === group.id ? "border-warning bg-warning/5" : "border-border hover:border-warning/50"
                        )}
                        onClick={() => setAgeGroup(group.id)}
                      >
                        <div className="flex items-center space-x-3">
                          <div className="w-4 h-4 border-2 border-warning rounded-full flex items-center justify-center">
                            {ageGroup === group.id && <div className="w-2 h-2 bg-warning rounded-full" />}
                          </div>
                          <div>
                            <p className="font-medium text-sm">{group.label}</p>
                            <p className="text-xs text-muted-foreground">{group.range}</p>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
              <div>
                <h2 className="text-xl font-semibold mb-1">Kids' Shoes</h2>
              ...
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  className="px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:border-warning"
                >
            ...
            {sortedProducts.length > 0 ? (
              ...
              <div className="text-center py-16">
                <div className="w-24 h-24 bg-gradient-warning/10 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Baby className="h-8 w-8 text-warning" />
                </div>
                <h3 className="text-lg font-semibold mb-2">No products found</h3>
                <p className="text-muted-foreground">
                  Try adjusting the filters to see more kids' products
                </p>
              </div>
            )}
